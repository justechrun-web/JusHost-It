...
| F.3 | Do you have a disaster recovery (DR) plan? | Yes. Our DR strategy is based on our use of Infrastructure as Code (Terraform) and managed cloud services. Our platform can be redeployed from scratch in a new GCP region if necessary. Databases have point-in-time recovery (PITR) enabled and are backed up automatically by GCP. |
